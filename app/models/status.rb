...
class Status < ApplicationRecord
  ...
  def self.story_type_dev_statuses
    where.not(name: ['not started', 'migrated', 'inactive', 'deleted'])
  end

  def self.article_type_dev_statuses
    where.not(name: ['not started', 'inactive', 'deleted'])
  end

  def self.scrape_task_dev_statuses
    where.not(name: ['not started', 'migrated', 'inactive', 'exported', 'on cron', 'deleted'])
  end
