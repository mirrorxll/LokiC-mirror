...
class StoryTypesTask < ApplicationTask
  private
  ...
  def send_report_to_editors_slack(iteration, url)
    ...
    SlackNotificationTask.new.perform(story_type.developer_fc_channel_name, message)
  end
  ...
  def opportunities_attached?(story_type, iteration)
    ...
    # slack message
    ::SlackNotificationTask.new.perform(developer, message)
    ::SlackNotificationTask.new.perform(manager, message)
