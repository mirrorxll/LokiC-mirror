...
class ScrapeTasksGrid
  ...
  column(:scrape_frequency, order: 'frequency.name')

  column(:data_set_location, header: 'Data Set location', mandatory: true)

  # column(:frequency, mandatory: true) { |record| record.first_export ? 'first' : 'follow up' }
