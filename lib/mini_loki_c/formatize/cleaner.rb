...
module MiniLokiC
  module Formatize
    ...
    d_opt = def_val(h[:d_opt], true) # IS THE DOT OPTIONAL?
                                         # BUILDING
    /#{"\\b" if separated}(#{w})#{"\\b" if separated}#{"\\.#{d_opt ? '*' : '+'}" if dot}/i
    end
    ...
    class Determiner
      ...
      private
      def load_names
        names_db = Mysql.on(DB01, 'il_raw')
        names_db.query('select name from popular_first_names').each{|row| @f_names << row['name'].strip.upcase}
        names_db.query('select name from popular_last_names order by count desc').each{|row| @l_names << row['name'].strip.upcase}
        names_db.close if names_db
      end
    end
