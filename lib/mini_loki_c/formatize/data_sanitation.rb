  ...
  module Formatize
    ...
    module DataSanitation
      ...
      def domain_name(value)
        value.gsub(/(?<=\w)\.(com|net|org|info|biz|mobi|asia|eu|xxx|us|co|mx|tw|gov|edu)\b/i){|capture| capture.downcase}
      end
      ...
      def normalize(value)
        ...
        value = value.gsub(/(?<!suite |No\. |Community |Apt\. )\ba\b(?!\.|\&|$)/i){|m| m.downcase}
        value = value.gsub(/\b(westbound|eastbound|northbound|southbound)\b/i){|m| m.downcase}
        value = value.gsub(/\b(iii?|iv|vii?i?|xv?ii?i?|xv|xiv|xix|xxi)\b|^([A-Z])/i){|m| m.upcase}
        BUSINESS_ACRONYMS_SUBS.each do |k, v|
